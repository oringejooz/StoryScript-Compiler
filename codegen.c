        ...
        if (sscanf(line, "TITLE \"%[^\"]\"", param1) == 1) {
            ...
            if (start_choices && end_choices && start_labels && end_labels) {
                ...
                *end_labels = '\0';   // Terminate labels part
                ...
                strncpy(choices_part, start_choices + 1, end_choices - start_choices - 1);
                ...
                // --- Corrected Choice Formatting ---
                char formatted_choices[MAX_LINE_LENGTH] = "";
                char *token = strtok(choices_part, ",");
                while (token != NULL) {
                    char temp[MAX_LINE_LENGTH];
                    snprintf(temp, sizeof(temp), "\"%s\", ", token); // Add quotes and format
                    strcat(formatted_choices, temp);
                    token = strtok(NULL, ",");
                }

                // Remove trailing comma and space
                if (strlen(formatted_choices) > 2) {
                    formatted_choices[strlen(formatted_choices) - 2] = '\0';
                }

                // Generate correct PROMPT format with properly quoted choices
                fprintf(target_file, "PROMPT %s -> [%s]\n", formatted_choices, labels_part);
            } else {
